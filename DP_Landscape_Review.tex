...
Despite this uncertainty, the scholarly communication community largely agrees on three essential properties of a data publication (Figure 1)\cite{smith_data_2009,sarah_callaghan_making_2012}.
...
Callaghan et al. (2012) draw a distinction between data that has been published (lower-case “p”) or formally Published (upper-case “P”): \textbf{published} data is simply available, while \textbf{Published} data has all four properties, including a guarantee of persistence\cite{sarah_callaghan_making_2012}.
Lawrence et al. (2011) define Publication as making data available following ``a procedure which results in the ability to make assertions about trustworthiness and fitness for purpose'' \cite{lawrence_data_2011}.
In this scheme, publication is in the sense of any kind of printed and distributed material, while Publication refers specifically to the peer-reviewed scholarly literature.
Here, we will use a single definition of published data: data that is available, documented to some extent, and citable, whatever the level of validation. 
...
Preparing documentation is frequently the most laborious step for researchers in taking data from useable within their lab to published
Incentivizing this effort is a major goal of data publication.
Given that the goal is to bring data into the scholarly record, documentation, which can be or at least look like a paper, is a natural hook.
...
Similarly, the five classes of data publication described by Lawrence et al.(2011) have recognizably different kinds of documentation\cite{lawrence_data_2011}.
...
Consequently, not all of the data collected must be published, and the documentation need not be sufficient for reuse for an unrelated purpose.
...
The strength of a data paper is in providing rich documentation, which is especially useful for non-standard `long-tail' research data.
...
An early implementer of data papers, \href{http://www.ijrr.org/}{The International Journal of Robotics Research}\cite{newman_data_2009} permits authors to host datasets on their own websites.

Data papers are predated by an approach that Lawrence et al. call \emph{data publication by proxy}, in which a paper providing a general description of a database or dataset serves as a citable proxy\cite{lawrence_data_2011}.
...
In an exceedingly complex landscape, academic, governmental, and commercial repositories publish data from diverse place- and interest-based research communities through diverse processes with and without linkage to the literature.
...
Although the guidlines are similar, review processes differ widely. 
...
Dynamic and complex datasets raise questions with multiple and conflicting answers.
Where the guidance of the print metaphor threatens to give out, it must be extended creatively.
Or, abandoned for another approach entirely.
...
\href{http://dat-data.com/}{Dat}, ``aim[s] to bring to data a style of collaboration similar to what git brings to source code.''
...
On the other hand, data publication targets this system explicitly, but leaves numerous other gaps.

Despite the degree of consensus around certain issues, data publication is still taking shape.
We expect that new approaches to sharing research data-- perhaps mixing metaphors, perhaps unique to data-- will continue to emerge over the next decade.
As a term, ``Data publication'' may open to cover such variegated approaches or hunch down and jostle next to other metaphors; the critical thing is to remain open to complexity and novelty.
