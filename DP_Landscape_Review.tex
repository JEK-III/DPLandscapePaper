...
The idea– and the ideal– that researchers should share data to advance knowledge and promote the common good is not new, but in recent years the conversation has shifted from sharing data to ``publishing'' data.  
...
Models of data publication can be classified in a variety of ways.
Lawrence (2011) describles five data publication models ``discriminated in the main by how the roles involved in publication are distributed between the various actors'' (such as the author, archive and journal).\cite{lawrence_data_2011}
...
The most prominent third-party repository specifically for making supplemental data public is Dryad\cite{dryad}, which accepts data underlying any peer-reviewed or otherwise ``reputable'' publication. 
...
However, scientific software is struggling for recognition just as data is, so software may not offer an existing reward system to tap into.
