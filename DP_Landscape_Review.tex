...
For instance, \textit{The Journal of Neuroscience} stopped publishing supplemental material in 2010\cite{maunsell_announcement_2010}. %generally - make sure journal titles are italicized, and "." comes after the citation at the end of a sentence.
...
The most extensive is the Trusted Repository Audit Checklist (TRAC)\cite{dale_trustworthy_2007} from the Center for Research Libraries; the checklist is widely used for reference, but certification is so onerous that only four repositories have gone thorough the process. %this last phrase seems a bit opinionanted. Maybe change wording a bit so it's less judgy?
...
Repositories run by governments or large universities are likely to be considered trustworthy (although the effects of the 2013 US government shutdown on PubMed might give one pause). %might need a reference here - esp. for international audiences. 
...
Or, consider a dataset composed of mRNA predictions drawn from an organism's genome that is periodically revised based on new experimental data and improved prediction methods. %might be good to cite a real dataset here... actual weather station data, or satellite data website...
...
However, at the same time that data validation seeks to model itself on literature peer-review, literature peer-review itself is in flux. %citation for this?
